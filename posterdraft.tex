...
\begin{poster}{
  ...
  {\includegraphics[width=18em]{Poster-images/ncl.jpg}}
  % Title
 ...
 The focus of the project is the curated branch of the database, which currently contains 424 models. These models have all been described in peer reviewed scientific literature.
 
 One of the languages used for submitting models to the databse and downloading models from the databse is Sytems Biology Markup Language (SBML).
 
 }
  
  \headerbox{R}{name=r, column=1,row=0}{
  R is a language and environment for statistical computing and graphics, providing a wide variety of statistical and graphical techniques. One of R's strengths is the ease with which well-designed publication-quality plots can be produced, including mathematical symbols and formulae where needed. 
  
  R is highly extensible via the installation of packages. An example is the package rsbml used to extract information from the model SBML files downloaded from the database.
  }
 ...
 SBML is a modelling standard used for exchanging models between different software tools. An example of SBML code is shown below. This code definines the species for one of the biomodels:
 ...
 Whilst SBML is easy for computers to generate and parse, it is difficult for humans to read and write. Since it is easier to work with R and interpret R output than SBML, R was used to extract the model information from the SBML files.
 }
...
In the project, the possibility of using SBO terms to track which models certain species apperared in was explored. The table below shows the frequency of the SBO terms for the species in the first model:
 ...
 \begin{multicols}{2}
 \includegraphics[trim= 1.5mm 5mm 4.5mm 5mm, clip, scale=0.42]{Poster-images/GlobalParametersHistogram.png}
  
  As shown in the graph above, the majority of models have 20 or less global parameters, with over 100 having less than 10 global parameters. This suggests that the models tend to have a low number of global parameters.

 
 \includegraphics[trim= 1.5mm 5mm 4.5mm 5mm, clip, scale=0.42]{Poster-images/LocalParametersHistogram.png}
 
 As shown in the graph above, the majority of models have 10 or less local parameters, due to a large proportion 0f models defining all their parameters as global parameters. This suggests that the models tend to have a low number of local parameters.
 \end{multicols}
 }
